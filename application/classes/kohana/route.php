...
class Kohana_Route {
	...
	/**
	 * @var  string Preview salt
	 */
	public static $preview_salt = FALSE;
	...
	{
		return Route::$_routes[$name] = new Route($uri_callback, $regex, $name);
	}
	 ...
	 * @uses    URL::site
	 */
	public static function url($name, array $params = NULL, $protocol = NULL, $region = TRUE)
	{
		$route = Route::get($name);

		// Create a URI with the route and convert it to a URL
		if ($route->is_external())
			return Route::get($name)->uri($params, $region);
		else
			return URL::site(Route::get($name)->uri($params, $region), $protocol);
	}
	 ...
	 */
	public function uri(array $params = NULL, $region = TRUE)
	{
		...
		if (strpos($uri, '<') === FALSE AND strpos($uri, '(') === FALSE)
		{
			...
			if ( ! $this->is_external()) {
				return $region ? self::set_region($uri, $region) : $uri;
			}
	...
	public static function set_region($uri, $region = NULL)
	{
		if (Request::current() == NULL)
			return $uri;
		$region = (empty($region) OR $region === TRUE)
			? Request::current()->site_code
			: $region;
		if (empty($region)) {
			return $uri;
		}
		...
		$_array = explode('#', $uri);
		$_left = Arr::get($_array, 0, '');
		$_right = Arr::get($_array, 1);
		$_right = empty($_right) ? '' : '#'.$_right;
		
		return $_left.$delimiter.'region='.$region.$_right;
	}
	...
	public static function get_preview_token($key)
	{
		if (is_array($key)) {
			$_key = '';
			foreach ($key as $v) {
		...
		return sha1($key.self::$preview_salt);
	}
	
	public static function check_preview_token($key, $preview_token)
	{
		if (empty($key))
			return false;
	
		return (self::get_preview_token($key) === $preview_token);
	}
