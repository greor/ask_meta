...
class Paginator {
	...
	public function get($link)
	{
		...
		$hash = '';
		if (($hash_pos = strpos($link, '#')) !== FALSE) 
		{
			$hash = substr($link, $hash_pos);
			$link = substr($link, 0, $hash_pos);
		}
		...
		for ($i = 1; $i <= $page_count; $i++)
		{
			...
			if ($this->_current == $i)
			{
				...
				if (($i - 1) > 0)
				{
					$return['previous'] = $link.($i - 1).$hash;
				}

				if (($i + 1) <= $page_count)
				{

					$return['next'] = $link.($i + 1).$hash;
				}
			}
		...
		$return['first_page'] = $link.'1'.$hash;
		$return['last_page'] = $link.$page_count.$hash;
