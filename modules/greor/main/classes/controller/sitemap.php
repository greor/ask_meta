...
class Controller_Sitemap extends Controller {
	...
	private $default_changefreq = 'weekly';
	private $default_priority = '0.5';
	private $default_separate_file = 0;
	
	protected $site_id;
	protected $site_id_master;
	private $domain = 'http://keks.fm';
	private $site_code;
	private $parsed_modules = array( 
		'cities', 'contacts', 'chart', 'feedback', 
		'news', 'playlist', 'programs', 'staff', 'search' 
	);
			...
			->and_where_open()
				...
				->or_where_open()
					...
					->and_where_open()
						...
						->or_where_open()
							->where('site_id', '=', $this->site_id_master)
							->and_where('for_all', '=', 1)
						->or_where_close()
					->and_where_close()
				->or_where_close()
	...
	private function _page_item($page)
	{
		...
		return array(
			'loc'        => Route::set_region($_page_loc, $_code),
	...
	private function _news_items($page)
	{
		...
		$db_categories = ORM::factory('news_Category')
			->exclude_hidden_elements( ($this->site_id == $this->site_id_master) )
			->find_all();
		...
		while ( ! $stop) {
			...
			if ($_item != FALSE) {
				...
				$return[] = array(
					'loc'        => str_replace(
						array( '{category_uri}', '{id}' ), 
						array( $_category_uri, $_item->id ), 
						$item_link_tpl
					),
					'lastmod'    => $_last_mod,
					'changefreq' => $page['sm_changefreq'],
	...
	private function set_default_values($item)
	{
		if (empty($item['sm_changefreq'])) {
			$item['sm_changefreq'] = $this->default_changefreq;
		}
		if (empty($item['sm_priority'])) {
			$item['sm_priority'] = $this->default_priority;
		}
		if ( ! isset($item['sm_separate_file']) OR $item['sm_separate_file'] === '' ) {
			$item['sm_separate_file'] = $this->default_separate_file;
		}
